...
then
    ...
    then
        ...
        /usr/bin/aws autoscaling create-auto-scaling-group --auto-scaling-group-name "AgileDeploymentToolkitAutoscalingGroup" --min-size ${minsize} --max-size ${maxsize} --instance-id ${id}
    fi  
fi
