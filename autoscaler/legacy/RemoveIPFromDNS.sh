#!/bin/sh
###################################################################################################################################
#Description: This script will remove the IP address passed as a parameter from the IP addresses registered with the DNS provider
#Author: Peter Winter
#Date: 12/01/2017
########################################################################################################################
# License Agreement:
...
if ( [ "${recordid}" != "" ] )
then
    /bin/echo "${0} `/bin/date`: Removing record with id ${recordid} from the dns and email address ${DNS_USERNAME}" >> ${HOME}/logs/MonitoringLog.log
