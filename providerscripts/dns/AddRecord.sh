...
# Author : Peter Winter
# Date   : 13/06/2016
# Description : This script will add a DNS record to the dns provider for a server
...
if ( [ "${dns}" = "cloudflare" ] )
then
    /bin/echo "${0} `/bin/date`: Adding record to the dns for ip address: ${ip}" >> ${HOME}/logs/MonitoringLog.log
