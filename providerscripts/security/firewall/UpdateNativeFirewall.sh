  ...
  # allips="`/bin/cat ${HOME}/runtime/ipsforfirewall`"
    ...
    then
       # /bin/touch ${HOME}/config/FIREWALL-UPDATING
        ...
        security_group_id="`/usr/bin/aws ec2 describe-security-groups | /usr/bin/jq '.SecurityGroups[] | .GroupName + " " + .GroupId' | /bin/grep adt-servers | /bin/sed 's/\"//g' | /usr/bin/awk '{print $NF}'`"

        if ( [ "${security_group_id}" = "" ] )
        then
            /usr/bin/aws ec2 create-security-group --description "This is the security group for your agile deployment toolkit server machines" --group-name "adt-servers" --vpc-id=${vpc_id}
            security_group_id="`/usr/bin/aws ec2 describe-security-groups | /usr/bin/jq '.SecurityGroups[] | .GroupName + " " + .GroupId' | /bin/grep  adt-servers | /bin/sed 's/\"//g' | /usr/bin/awk '{print $NF}'`"
        fi
       ...
       then
           for ip in ${alldnsproxyips}
           do
               /usr/bin/aws ec2 authorize-security-group-ingress --group-id ${security_group_id} --ip-permissions IpProtocol=tcp,FromPort=443,ToPort=443,IpRanges="[{CidrIp=${ip}/32}]" 2>/dev/null
           done
       else
